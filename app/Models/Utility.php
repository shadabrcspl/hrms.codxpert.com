...
class Utility extends Model
{
    ...
    /**
     * Get shift window for a given date and shift type
     *
     * @param string $shiftType  day | night
     * @param string|Carbon $baseDate  Date you want attendance for (Y-m-d)
     * @return array [Carbon $start, Carbon $end]
     */
    public static function getShiftWindow(string $shiftType, $baseDate): array
    {
        $baseDate = $baseDate instanceof Carbon
            ? $baseDate->copy()
            : Carbon::parse($baseDate);

        if ($shiftType === 'night') {

            // Night shift: 12 PM → next day 12 PM
            $start = $baseDate->copy()->setTime(12, 0, 0);
            $end   = $start->copy()->addDay();

        } else {

            // Day shift: 12 AM → 11:59:59 PM
