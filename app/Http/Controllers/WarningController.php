...
class WarningController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //   return redirect()->route('warning.index')->with('success', __('Warning  successfully created.') . ((!empty($resp) && $resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
           ...
           if($setings['new_warning'] == 1 && (\Auth::user()->type == 'employee' || \Auth::user()->type == 'hr' ))
            {
                $warning_by = Employee::find( $warning->warning_by);
                $warning_to = Employee::find($warning->warning_to);
                $company_email = User::where('id',$warning_by->created_by )->first()->email;
                $uArr = [
                    'warning_by'=>$warning_by->name,
                    'warning_to'=> $warning_to ->name,
                    'warning_subject'=>$warning->subject,
                    'warning_detail'=>$warning->description,
                    'warning_date'=>$request->warning_date,
                ];
                $resp = Utility::sendEmailTemplate('new_warning', [$company_email], $uArr);
