...
class AttendanceEmployeeController extends Controller
{
        ...
        try {
            if (\Auth::user()->can('Manage Attendance')) {
                ...
                if (\Auth::user()->type == 'employee') {
                    ...
                    $attendanceEmployee = $attendanceEmployee->groupBy('date')->get();
                    dd('In Employee',$attendanceEmployee);
                } else {
    
                    /* $monthInput = $request->input('month');
                    $attendanceDate = date('Y-m-d', strtotime($monthInput . '-01'));
                    $employee = Employee::select('id', 'shift_type')->where('created_by', \Auth::user()->creatorId());
                    ...
                    // RONAK STARTED FROM HERE
                    // Total Working Hours Calculation

                    $monthInput = $request->input('month');
                    $attendanceDate = date('Y-m-d', strtotime($monthInput . '-01'));

                    $employeeQuery = Employee::select('id', 'shift_type', 'name')->where('created_by', \Auth::user()->creatorId());
                    ...
                    $employeesShift = $employees->keyBy('id');

                    $attendanceQuery = AttendanceEmployee::with('employee:id,shift_type,name')->whereIn('employee_id', $employeeIds);
                    ...
                    /* if ($request->type == 'monthly' && !empty($request->month)) {
                        ...
                        // dd($start_date, $end_date, $employeeIds);
                        $attendanceRecords->whereBetween('clock_in', [$start_date, $end_date]);

                    } elseif ($request->type == 'daily' && !empty($request->date)) {

                        $attendanceRecords->whereDate('clock_in', $request->date);
                   ...
                   if ($request->type === 'monthly' && !empty($request->month)) {
                        ...
                        // SAFE SUPERSET (covers all shifts)
                        $start = $month->copy()->startOfMonth()->subDay()->setTime(12, 0, 0);
                        $end   = $month->copy()->endOfMonth()->addDay()->setTime(12, 0, 0);

                        $attendanceQuery->whereBetween('clock_in', [$start, $end]);

                    } elseif ($request->type === 'daily' && !empty($request->date)) {

                        $date = Carbon::parse($request->date);

                        // SAFE superset (no shift assumptions)
                        $start = $date->copy()->startOfDay();              // D 00:00
                        $end   = $date->copy()->addDay()->setTime(12, 0);  // D+1 12:00
                        ...
                        $attendanceQuery->whereBetween('clock_in', [
                            $today->copy()->startOfDay(),
                            $today->copy()->endOfDay()
                        ]);
                    }

                    $attendanceRecords = $attendanceQuery->orderBy('clock_in')->get();

                    $attendanceEmployee = $attendanceRecords
                        ->groupBy('employee_id')
                        ->map(function ($employeeRecords, $employeeId) use ($employees, $employeesShift, $request) {
                            
                            $shiftType = $employeesShift[$employeeId]->shift_type ?? 'day';

                            return $employeeRecords
                                ->groupBy(function ($row) use ($shiftType, $employeeId) {
                                    // Determine correct SHIFT DATE for this record
                                    if ($shiftType === 'night') {
                                        // Before 12 PM belongs to previous day
                                        return $row->clock_in->lt(
                                            $row->clock_in->copy()->setTime(12, 0, 0)
                                        )
                                            ? $row->clock_in->copy()->subDay()->toDateString()
                                            : $row->clock_in->toDateString();
                                    }

                                    // Day shift
                                    return $row->clock_in->toDateString();
                                })
                                ->map(function ($shiftGroup, $shiftDate) use ($employeeId, $shiftType, $request) {
                                    
                                    // Filter by requested date for daily type
                                    if ($request->type === 'daily' && !empty($request->date)) {
                                        $requestedDate = Carbon::parse($request->date)->toDateString();
                                        if ($shiftDate !== $requestedDate) {
                                            return null; // Skip records not for requested date
                                        }
                                    }

                                    // Sum working_hours
                                    $totalSeconds = $shiftGroup->sum(function ($row) {
                                        if (!$row->working_hours) return 0;
                                        [$h, $m, $s] = explode(':', $row->working_hours);
                    ...
                    /* $attendanceEmployee = $attendanceRecords
                        ->groupBy(fn ($row) =>
                            $row->employee_id . '_' . $row->clock_in->toDateString()
                        )
                        ->map(function ($group) use ($employeesShift) {

                            $record = $group->first();
                            $employeeId = $record->employee_id;

                            // ✅ shift_type per employee
                            $shiftType = $employeesShift[$employeeId]->shift_type ?? 'day';

                            // ✅ get correct shift window for THIS employee
                            [$start, $end] = Utility::getShiftWindow(
                                $shiftType,
                                $record->clock_in->toDateString()
                            );

                            // ✅ keep only records belonging to this shift window
                            $validRecords = $group->filter(function ($row) use ($start, $end) {
                                return $row->clock_in->between($start, $end);
                            });
                            ...
                            $firstClockIn = $validRecords->min('clock_in');
                            $lastClockOut = $validRecords->max('clock_out');

                            // ✅ SUM working_hours
                            $totalSeconds = $validRecords->sum(function ($row) {
                            ...
                            return (object) [
                                ...
                                'date'                => $start->toDateString(), // shift date
                                'shift_type'          => $shiftType,
                    ...
                    /* $attendanceEmployee = $attendanceRecords
                        ...
                        ->map(function ($group) {
                            ...
                            // First in & last out for the day
                            $firstClockIn = $group->min('clock_in');
                            $lastClockOut = $group->max('clock_out');

                            if (!$firstClockIn || !$lastClockOut) {
                                return null;
                            }

                            // DATETIME → simple diff (night shift handled automatically)
                            // $totalSeconds = $firstClockIn->diffInSeconds($lastClockOut);

                            // ✅ SUM working_hours (TIME) safely
                            $totalSeconds = $group->sum(function ($row) {
                                if (empty($row->working_hours)) {
                                    return 0;
                                }

                                [$h, $m, $s] = array_map('intval', explode(':', $row->working_hours));
                                return ($h * 3600) + ($m * 60) + $s;
                            });
                            ...
                            return (Object) [
                                ...
                                'status'              => $record->status,
                                'employee'            => $record->employee,
                                'date'                => $firstClockIn->toDateString(),
    ...
    public function update(Request $request, $id)
    {
        ...
        if (Auth::user()->type == 'employee') {
            ...
            $clockIn  = Carbon::parse($request->date . ' ' . $request->clock_in . ':00');
            $clockOut = Carbon::parse($date . ' ' . $time . ':00');

            // If clock-out is earlier than clock-in → crossed midnight (night shift)
            ...
            $attendanceEmployee['clock_out']          = Carbon::parse($date . ' ' . $time);
            $attendanceEmployee['working_hours']      = gmdate('H:i:s', $totalSeconds);
            ...
            // Assume these are provided / known
            $now = Carbon::now(); // actual clock-out time
            $clockIn = Carbon::parse($attendanceEmployee->clock_in);

            // dd($todayAttendance->clock_in);
            // SHIFT END TIME (example: 19:00 for day shift, 12:00 for night shift)
            // IMPORTANT: build this as DATETIME, not time string
            // Shift end datetime (example: $endTime = '19:00:00' or '12:00:00')
            $shiftEnd = Carbon::parse($attendanceEmployee->clock_in->toDateString() . ' ' . $endTime);

            // Night-shift safety: if shift end < clock in → next day
            if ($shiftEnd->lt($clockIn)) {
                $shiftEnd->addDay();
            }
            ...
            // ----------------------
            if ($now->lt($shiftEnd)) {
                $earlySeconds = $now->diffInSeconds($shiftEnd);
            ...
            // ----------------------
                ...
                $overtimeSeconds = $shiftEnd->diffInSeconds($now);
                $overtime = gmdate('H:i:s', $overtimeSeconds);
                $earlyLeaving = '00:00:00';
            }

            // ----------------------
            // CLOCK-OUT & WORKING HOURS
            // ----------------------
            $clockOut = $now;
